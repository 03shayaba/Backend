      ...
      {!isEmailSent &&
        <form onSubmit={onSubmitEmail} action="" className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
          ...
          <div className="mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]">
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder='Email -id' required className='bg-transparent outline-none text-white' />
          </div>
          ...
          <div onPaste={handlePaste} className='flex justify-between mb-8'>
            {
              Array(6).fill(0).map((_, index) => (
                <input type="text" maxLength='1' key={index} required className='w-11 h-12 bg-[#333A5C]  text-white  text-center text-xl rounded-md'
      ...
      {isEmailSent && isOtpSubmitted &&
        <form onSubmit={onSubmitNewPassword} action="" className='bg-slate-900 p-8 rounded-lg shadow-lg w-96 text-sm'>
          ...
          <div className="mb-4 flex items-center gap-3 w-full px-5 py-2.5 rounded-full bg-[#333A5C]">
            ...
            <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)} placeholder='Enter password' required className='bg-transparent outline-none text-white' />
          </div>
          <button className='w-full py-2.5 bg-gradient-to-r from-indigo-500 to-indigo-900 text-white rounded-full mt-3'>Submit</button>
